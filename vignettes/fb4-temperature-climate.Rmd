...
.pc <- readRDS(system.file("extdata/temperature_climate_precomputed.rds",
                           package = "fb4package"))
sens_df     <- .pc$sens_df
scenario_df <- .pc$scenario_df
perf_df     <- .pc$perf_df
p_fixed     <- .pc$p_fixed
target_wt   <- .pc$target_wt
...
```{r manual-sensitivity, eval=FALSE}
...
```{r manual-sensitivity-table, echo=FALSE}
knitr::kable(
  sens_df,
...
```{r sensitivity-plot-img, echo=FALSE, fig.cap="Built-in temperature sensitivity plot. Each point shows the p required to achieve the target weight at a given temperature offset."}
knitr::include_graphics("temperature_sensitivity_plot.png")
```
...
```{r climate-scenarios, eval=FALSE}
p_fixed   <- 0.65
...
```{r climate-scenarios-table, echo=FALSE}
knitr::kable(
  scenario_df,
...
```{r thermal-window-output, echo=FALSE}
...
text(max(perf_df$Temperature) - 2, 1.02, "p = 1 (maximum ration)", col = "firebrick", cex = 0.8)
grid()
```
