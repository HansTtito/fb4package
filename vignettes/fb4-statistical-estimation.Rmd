...
# Load pre-computed results so the vignette builds quickly on CRAN.
# To regenerate, run data-raw/precompute_stat_estimation_vignette.R
.pc <- readRDS(system.file("extdata/stat_estimation_precomputed.rds",
                           package = "fb4package"))
res_bs    <- .pc$res_bs
res_optim <- .pc$res_optim
res_mle   <- .pc$res_mle
res_boot  <- .pc$res_boot
res_ref   <- .pc$res_ref
ref_p    <- .pc$ref_p
mr_data  <- .pc$mr_data
res_hier <- .pc$res_hier
n_fish   <- .pc$n_fish
true_p   <- .pc$true_p
rm(.pc)
```
...
| `"optim"` | target final weight | point estimate of *p* + convergence info |
...
## Strategy 2 — Optimisation (`optim`)

`optim` delegates the search to R's `stats::optim()` (default method: Brent).
Unlike binary search, which brackets a root, `optim` minimises the absolute
difference between the simulated and target weight as a continuous objective
function. It returns the same point estimate of *p* but also reports
convergence status and the number of function evaluations used.

Use `optim` when you want a drop-in replacement for binary search that
leverages gradient or derivative-free optimisation, or when you need the
Hessian for downstream uncertainty quantification.

```{r optim, eval=FALSE}
res_optim <- run_fb4(bio,
                     strategy     = "optim",
                     fit_to       = "Weight",
                     fit_value    = 3000,
                     optim_method = "Brent",
...
cat("p estimate  :", round(res_optim$summary$p_value,      4), "\n")
cat("Final weight:", round(res_optim$summary$final_weight, 1), "g\n")
cat("Converged   :", res_optim$summary$converged, "\n")
```

---

## Strategy 3 — Maximum Likelihood Estimation
...
## Strategy 5 — Hierarchical model (mark-recapture)
...
### Strategies 1–4: single-population estimates

Binary search, optim, MLE, and bootstrap all answer the same question —
*what p produces the observed growth of a representative fish?* — and can
therefore be compared numerically. All four use the same `bio` object
(`initial_weight = 1800 g`, target or observed weights ≈ 3 000 g).

```{r compare-1234, echo=FALSE}
.get <- function(x, field) { v <- x$summary[[field]]; if (is.null(v)) NA_real_ else v }

comp1234 <- data.frame(
  Strategy    = c("binary_search", "optim", "mle", "bootstrap"),
  p_estimate  = round(c(.get(res_bs,   "p_value"),
                        .get(res_optim, "p_value"),
                        .get(res_mle,  "p_value"),
                        .get(res_boot, "p_mean")), 4),
  Uncertainty = round(c(NA_real_, NA_real_,
                        .get(res_mle,  "p_se"),
                        .get(res_boot, "p_sd")), 4),
...
knitr::kable(comp1234,
             col.names = c("Strategy", "p estimate", "SE / SD",
                           "Uncertainty type"),
             align  = c("l", "r", "r", "l"),
             caption = "Strategies 1–4 fitted to the same data (Chinook salmon, 100-day simulation, target weight ≈ 3 000 g).")
```

### Strategy 5: population-level distribution
