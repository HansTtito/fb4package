...
#' Generar parámetros de ejemplo para mortalidad y reproducción
#'
#' Crea parámetros típicos para diferentes tipos de especies
#'
#' @param species_type Tipo de especie ("small_fish", "large_fish", "general")
#' @param include_reproduction Incluir parámetros reproductivos
#' @param reproductive_season Temporada reproductiva ("spring", "summer", "fall", "winter")
#' @return Lista con parámetros de ejemplo
#' @export
generate_example_mortality_reproduction_params <- function(species_type = "general",
                                                           include_reproduction = TRUE,
                                                           reproductive_season = "spring") {
  
  # Parámetros de mortalidad por tipo de especie
  if (species_type == "small_fish") {
    mortality_params <- list(
      base_mortality = 0.003,        # 0.3% diaria
      natural_mortality = 0.003,
      fishing_mortality = 0.001,
      predation_mortality = 0.002,
      weight_threshold = 5,          # 5g peso crítico
      starvation_factor = 10,
      optimal_temp = 22,
      thermal_tolerance = 8,
      stress_factor = 3
    )
  } else if (species_type == "large_fish") {
    mortality_params <- list(
      base_mortality = 0.001,        # 0.1% diaria
      natural_mortality = 0.001,
      fishing_mortality = 0.002,
      predation_mortality = 0.0005,
      weight_threshold = 50,         # 50g peso crítico
      starvation_factor = 5,
      optimal_temp = 18,
      thermal_tolerance = 12,
      stress_factor = 2
    )
  } else {  # general
    mortality_params <- list(
      base_mortality = 0.002,        # 0.2% diaria
      natural_mortality = 0.002,
      fishing_mortality = 0.001,
      predation_mortality = 0.001,
      weight_threshold = 20,         # 20g peso crítico
      starvation_factor = 7,
      optimal_temp = 20,
      thermal_tolerance = 10,
      stress_factor = 2.5
  ...
  result <- list(mortality = mortality_params)
  
  # Parámetros reproductivos si se solicitan
  if (include_reproduction) {
    
    # Determinar día pico según temporada
    peak_day <- switch(reproductive_season,
                       "spring" = 120,   # Finales de abril
                       "summer" = 200,   # Julio
                       "fall" = 280,     # Octubre
                       "winter" = 30     # Enero
    )
    
    # Generar patrón reproductivo
    spawn_pattern <- generate_reproduction_pattern(
      days = 1:365,
      peak_day = peak_day,
      duration = 45,
      max_spawn_fraction = if (species_type == "small_fish") 0.20 else 0.15,
      pattern_type = "gaussian"
    )
    
    reproduction_params <- list(
      spawn_pattern = spawn_pattern,
      energy_density = if (species_type == "small_fish") 4500 else 5500,
      reproductive_season = reproductive_season,
      peak_day = peak_day
    )
    
    result$reproduction <- reproduction_params
  }
