...
#'
#' @description
#' Provides safe versions of common mathematical operations with
#' built-in validation and error handling.
#'
#' @param x Input value(s)
#' @param operation Mathematical operation ("sqrt", "exp", "log")
#' @param param_name Parameter name for error messages
#' @param max_exp Maximum exponent for exp operations
#' @param min_val Minimum value for sqrt/log operations
#' @param strategy Handling strategy for problematic values
#'
#' @return List with result and validation information
#' @keywords internal
safe_math_core <- function(x, operation, param_name = "value", max_exp = 700,
                          min_val = 0, strategy = "clamp") {
  
  # Validate input first
  validation <- validate_numeric_core(x, param_name, strategy = strategy)
  
  if (!validation$valid && strategy == "strict") {
    return(list(result = x, validation = validation))
  }
  ...
  if (operation == "sqrt") {
    ...
    negative_mask <- !is.na(x) & is.finite(x) & (x < min_val)
    if (any(negative_mask)) {
      if (strategy == "clamp") {
        result_value[negative_mask] <- min_val
        validation$warnings <- c(validation$warnings,
                                sprintf("safe_sqrt: %d negative %s(s) clamped to %.3f",
                                        sum(negative_mask), param_name, min_val))
      }
    }
    
    # Apply sqrt to valid values
    valid_mask <- !is.na(result_value) & is.finite(result_value)
    result_value[valid_mask] <- sqrt(result_value[valid_mask])
    
  } else if (operation == "exp") {
    # Handle overflow
    overflow_mask <- !is.na(x) & is.finite(x) & (x > max_exp)
    if (any(overflow_mask)) {
      if (strategy == "clamp") {
        result_value[overflow_mask] <- max_exp
        validation$warnings <- c(validation$warnings,
                                sprintf("safe_exp: %d %s(s) clamped to max %.1f",
                                        sum(overflow_mask), param_name, max_exp))
      }
    }
    
    # Handle underflow
    underflow_mask <- !is.na(x) & is.finite(x) & (x < -max_exp)
    if (any(underflow_mask)) {
      result_value[underflow_mask] <- -max_exp
      if (strategy == "clamp") {
        validation$warnings <- c(validation$warnings,
                                sprintf("safe_exp: %d %s(s) set to underflow",
                                        sum(underflow_mask), param_name))
      }
    }
    
    # Apply exp to valid values
    valid_mask <- !is.na(result_value) & is.finite(result_value)
    result_value[valid_mask] <- exp(result_value[valid_mask])
    
  } else if (operation == "log") {
    # Handle non-positive values
    nonpositive_mask <- !is.na(x) & is.finite(x) & (x <= 0)
    if (any(nonpositive_mask)) {
      if (strategy == "strict") {
        validation$valid <- FALSE
        validation$errors <- c(validation$errors, 
                              paste("log: non-positive values in", param_name))
        return(list(result = x, validation = validation))
      } else {
        result_value[nonpositive_mask] <- NA_real_
        validation$warnings <- c(validation$warnings,
                                sprintf("log: %d non-positive %s(s) set to NA",
                                        sum(nonpositive_mask), param_name))
      }
    }
    
    # Apply log to valid values
    valid_mask <- !is.na(result_value) & is.finite(result_value) & (result_value > 0)
    result_value[valid_mask] <- log(result_value[valid_mask])
  }
  
  return(list(result = result_value, validation = validation))
}
