...
#' Validar resultados FB4
#'
#' @param fb4_result Objeto fb4_result
#' @return Lista con resultados de validación
#' @export
validate_fb4_results <- function(fb4_result) {
  ...
  if (!fb4_result$model_info$fit_successful) {
    validation$warnings <- c(validation$warnings, "El ajuste del modelo no fue exitoso")
  }

  # Verificar balance energético
  if (!fb4_result$energy_balance$balanced) {
    validation$warnings <- c(validation$warnings,
                             paste("Balance energético no equilibrado. Error relativo:",
                                   round(fb4_result$energy_balance$relative_error * 100, 4), "%"))
  }

  # Verificar datos diarios si existen
  if (!is.null(fb4_result$daily_output)) {
    daily_data <- fb4_result$daily_output

    # Verificar pesos negativos
    if (any(daily_data$Weight.g <= 0, na.rm = TRUE)) {
      validation$errors <- c(validation$errors, "Pesos negativos o cero detectados")
      validation$valid <- FALSE
    }

    # Verificar tasas de crecimiento extremas
    extreme_growth <- abs(daily_data$Specific.Growth.Rate.g.g.d) > 0.1
    if (any(extreme_growth, na.rm = TRUE)) {
      validation$warnings <- c(validation$warnings, "Tasas de crecimiento extremas detectadas (>10% por día)")
    }

    # Verificar consumo
    if (any(daily_data$Consumption.g < 0, na.rm = TRUE)) {
      validation$errors <- c(validation$errors, "Consumo negativo detectado")
      validation$valid <- FALSE
    }

    # Verificar temperatura
    if (any(daily_data$Temperature.C < -5 | daily_data$Temperature.C > 50, na.rm = TRUE)) {
      validation$warnings <- c(validation$warnings, "Temperaturas fuera de rango típico detectadas")
    }
  }

  # Verificar p-value
  if (!is.na(fb4_result$model_info$p_value)) {
    if (fb4_result$model_info$p_value < 0.001) {
      validation$warnings <- c(validation$warnings, "p-value muy bajo (<0.001)")
    } else if (fb4_result$model_info$p_value > 4.99) {
      validation$warnings <- c(validation$warnings, "p-value muy alto (>4.99)")
    }
  }

  validation$n_warnings <- length(validation$warnings)
  validation$n_errors <- length(validation$errors)
